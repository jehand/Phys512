...
def integrate_step(fun,x1,x2,tol): 
    ...
    n = 0 #Add a term n to count the number of times we call fun.
    n += len(x)
...
"""
def integrator(f,x1,x2,tol,n=0,y=[],xm=0):
    x = np.linspace(x1,x2,5)
    ...
    if myerr < tol:
        ...
        xm = 0.5*(x1+x2)
        a1,n1 = integrator(f,x1,xm,tol/2,n,y,xm)
        a2,n2 = integrator(f,xm,x2,tol/2,n,y,xm)
        return a1+a2, n1+n2+n