...
"""
def integrator(f,x1,x2,tol,y1=0,ym=0,y2=0,n=0):
    x = np.linspace(x1,x2,5)
    if n == 0: #If has been evaluated 0 times, then it evaluates f(x) at 5 points
        y = f(x)
        y1,midl,ym,midr,y2 = y[0],y[1],y[2],y[3],y[4] #Defining all the variables so that the function values are stored each iteration
        n = len(x)
    else: #If has been evaluated >0 times, then it just calculates the two new values that it needs to
        midl = f(x[1]) #Re-calculating the two points on either side of ym that need to be calculated
        midr = f(x[3])
        n = 2
    area1 = (x2-x1)*(y1+4*ym+y2)/6
    area2 = (x2-x1)*(y1+4*midl+2*ym+4*midr+y2)/12
    myerr = np.abs(area1-area2)
    if myerr < tol:
        return area2, n
    else:
        xm = 0.5*(x1+x2)
        a1,n1 = integrator(f,x1,xm,tol/2,y1,midl,ym,n) #Changing what we use as y1,ym and y2 as mentioned above
        a2,n2 = integrator(f,xm,x2,tol/2,ym,midr,y2,n)
        return a1+a2, n1+n2+n