...
alpha = -1.1 #for our power law
saveplot = True #to save plots when we want
...
#assert(np.max(probpower) <= 1) #This does not holds as well because the power distribution cannot be a bounding distribution
...
y = np.exp(-x)
plt.hist(lorentznumbs[acceptlorentz], density=True, bins=100, range=(0,5), histtype="bar", color="teal", ec="black",label="Histogram")
...
plt.hist(gaussnumbs[acceptgauss], density=True, bins=100, range=(0,5), histtype="bar", color="darkkhaki", ec="black",label="Histogram")
plt.plot(x,y,"r-",label="Predicted")
plt.xlabel("Number", fontsize=14)
plt.ylabel("Relative Frequency", fontsize=14)
plt.legend(fontsize=12)
...
y = np.exp(-x)
plt.hist(powernumbs[acceptpower], density=True, bins=100, range=(1,6), histtype="bar", color="cornflowerblue", ec="black",label="Histogram")
